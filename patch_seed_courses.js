...
// Helper to patch single lesson in a file content string
function patchFileContent(content, isSeedFile) {
  let patched = content;
  for (const item of updates) {
    if (isSeedFile) {
      // For seed.ts, match duration and add details if not present, or replace
      // Pattern: id: 'c1-l6', ... duration: '18:50', isPreview: false
      const regex = new RegExp(
        `(\\{\\s*id:\\s*'${item.id}',\\s*chapterId:\\s*ch1_\\d\\.id,\\s*title:\\s*['"]Buổi \\d+:[^'"]+['"],\\s*duration:\\s*['"])[^'"]+(['"],\\s*isPreview:\\s*\\w+)(\\s*\\})`,
        'g'
      );
      ...
      // For data/courses.ts, match duration and replace/add fields
      const regex = new RegExp(
        `(\\{\\s*id:\\s*'${item.id}',\\s*title:\\s*['"]Buổi \\d+:[^'"]+['"],\\s*duration:\\s*['"])[^'"]+(['"],\\s*isPreview:\\s*\\w+)(\\s*\\})`,
        'g'
      );
      const replacement = `$1${item.duration}$2, videoUrl: '${item.videoUrl || ''}', documentUrl: '${item.documentUrl || ''}', textContent: \`${item.textContent.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`$3`;
      patched = patched.replace(regex, replacement);
